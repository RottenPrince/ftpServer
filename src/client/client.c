...
int main() {
  ...
  for (p = servinfo; p != NULL;
       p = p->ai_next) { // Loop through all the results and connect to the
                         // first we can
    ...
    if (connect(sockfd, p->ai_addr, p->ai_addrlen) ==
        -1) { // Fixed the connect() function call
      close(sockfd);
  ...
  while (1) {
    ...
    if (strcmp(cmd, "quit") == 0) {
      ...
      char *filePath = "./src/client/Downloaded/";

      FILE *fp = fopen(concat(filePath, filename), "wb");
      if (fp == NULL) {
        perror("Error opening file");
        continue;
      }
      int total_bytes_received =
          0;              // Initialize to keep track of total bytes received
      int bytes_received; // Receive file contents
      while ((bytes_received = recv(sockfd, file_buffer, BUFFERSIZE, 0)) > 0) {
        ...
        if (bytes_received <
            BUFFERSIZE) { // Check if we've received the file completely
                          // (optional, based on your protocol)
