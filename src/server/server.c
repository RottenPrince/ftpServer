...
#define MSG_SIZE 100  // max size of the message, 100 because the client can only recieve 100 bytes of data


// Server must become a FTP server
// on command:
// 	@dir would provide the list of existing files that are eligible for download
// 	@get would fetch file or files and send to the client that requested them
...
int main()
{
    ...
    rv = getaddrinfo(NULL, PORT, &hints, &servinfo); // Retrieve address information for the localhost and specified port
	if (rv != 0) {
  ...
  // Set up the socket for listening
	...
	printf("server: waiting for a connection...\n"); // Print message indicating waiting for connection
	...
	inet_ntop(their_addr.ss_family, get_in_addr((struct sockaddr *)&their_addr), s, sizeof s); // Get the IP address of the client and store it in s
    ...
    int send_result = send(new_fd, msg, strlen(msg), 0); // Send the message back to client
    if (send_result == -1) {
        perror("send"); // Print error message if sending data fails
    }

    close(new_fd); // Done sending message, close connection to client
